"""Generate EPS, cash flow, and revenue summaries from SEC company facts."""
...
failed = 0
failed_list = []
# Required by the SEC: always include a descriptive User-Agent when pulling
# bulk data.
headers = {
...
def to_10_digits(n) -> str:
    """Return ``n`` padded to the standard 10-digit CIK string."""
...
for x in response_tickers:
    try:
        # ``response_tickers`` contains every known ticker; we simply iterate
        # through the list, loading any cached facts payloads that already exist
        # on disk.
        ...
        def add_q(json: dict, year: int, value: int | float, quarter: str):
            """Ensure the ``years``/quarter structure exists before assignment."""

            json.setdefault("years", {}).setdefault(year, {})[quarter] = value
        def eps(start_year, eps_json):
            """Parse quarterly EPS values for ``start_year`` from the facts feed."""
                ...
                if x['form'] == '10-q' or x['form'] == '10-k':
                    ...
                    try:
                        ...
                        try:
                            ...
                            # When a ``frame`` value is absent we fall back to parsing the
                            # ``fp`` plus ``start``/``end`` fields to infer the right quarter.
                            if x['fp'] == 'q1':
                                    if f"{start_year}-01" in x['start']:
                                        if f"{start_year}-03" in x['end']:
                                            q1_eps = x['val']
                                            add_q(eps_json, start_year, q1_eps, quarter="q1")
                                            #print(f"came from: {[x]} date parse")
                    ...
                    try:
                        ...
                        try:
                            ...
                            # Fall back to the ``fp`` field if the cumulative frame is
                            # missing, mirroring the quarter-by-quarter logic above.
                            if x['fp'] == 'q2':
                                    if f"{start_year}-01" in x['start']:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # As with Q1/Q2, infer the quarter's contribution when only
                            # fiscal period metadata is available.
                            if x['fp'] == 'q3':
                                    if f"{start_year}-01" in x['start']:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # ``fp`` equals FY for the year-end report; derive the
                            # quarterly number by subtracting the year-to-date total.
                            if x['fp'] == 'FY':
                                    if f"{start_year}-01" in x['start']:
                    ...
                    pass
                    #print("not 10-q or 10-k: ", x['form'])
            #print(eps_json)
            return eps_json

        def cashflow(start_year, cashflow_json):
            """Derive quarterly operating cash flow figures from cumulative data."""
        ...
        def rev(response):
            """Collect all revenue frames (across various GAAP labels)."""

            # ``all_frame_rev`` keeps entries as "value_frame" strings so that a
            # later pass can deduplicate quarters and select the best number for
            # each period.
            all_frame_rev = []
        ...
        def rev_graph(start_year, all_frame_rev, revenue_json):
            """Select the best revenue values per quarter and store them."""
            ...
            # Normalize the quarter labels so the later dedupe logic can process
            # them chronologically, even if the SEC payload orders them
            # inconsistently.
            ...
            for x in unique_list:
                ...
                #print(f"largest rec number is: {largest_rev} using {largest_rev} for frame {x}")
        ...
        # Iterate through each fiscal year we care about and populate the
        # derived metric dictionaries.  The try/except blocks make the parser
        # tolerant of missing data for smaller issuers.
        while True:
            try: 
        ...
        # Persist the derived metrics so later scripts (like ``deltas.py``) can
        # read them without recomputing everything.
